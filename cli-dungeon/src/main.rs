...
async fn handle_encounter(pool: &Pool, character_info: &CharacterInfo) {
    loop {
        ...
        let player_character = cli_dungeon_core::character::get_character(pool, character_info)
            .await
            .unwrap();
        ...
        let outcome = take_turn(pool, character_info, action, bonus_action, target, target)
            .await
            .unwrap();
        display_turn_outcome(outcome);
    }
